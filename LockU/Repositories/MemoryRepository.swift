...
final class MemoryRepository: ObservableObject {
    ...
    func saveImage(
        ...
        try captureWorkflow.execute(CaptureMemoryRequest(image: image, createdAt: createdAt, filterID: filterID, weather: weather, captureMode: captureMode, imageStyle: imageStyle, dailyFilm: dailyFilm, memoryNote: memoryNote, moodEmoji: moodEmoji, origin: .dailyCapture, importedAt: nil)).memory
    }

    @discardableResult
    func importLegacyImage(_ image: UIImage, createdAt: Date) throws -> MemoryRecord {
        try createImageRecord(CaptureMemoryRequest(image: image, createdAt: createdAt, filterID: nil, weather: nil, captureMode: .legacy, imageStyle: .original, dailyFilm: nil, memoryNote: nil, moodEmoji: nil, origin: .legacy, importedAt: nil), enforceDailyLimit: false)
    }
    ...
    func saveDualCameraMemory(
        ...
        let normalizedMoodEmoji = try MemoryMoodEmojiPolicy.normalized(moodEmoji)
    ...
    private func loadImageAsync(
        ...
        let cacheKey = "\(fileName)-\(purpose.rawValue)-\(pixelWidth)x\(pixelHeight)"
        if let cached = imageCache.image(forKey: cacheKey) { return cached }
        guard let url = imageStorage.url(fileName: fileName) else { return nil }
        let maximumPixelDimension = CGFloat(max(pixelWidth, pixelHeight))
        let loadTask = Task.detached(priority: .userInitiated) {
            SendableImage(image: MemoryImageStorage.downsampledImage(at: url, targetPixelSize: maximumPixelDimension))
        }
        ...
        guard !Task.isCancelled, let image = loaded.image else { return nil }
        imageCache.insert(image, forKey: cacheKey, cost: image.lockUApproximateStorageCost)
        return image
    }
